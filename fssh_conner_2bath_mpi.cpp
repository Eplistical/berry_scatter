...
void init_state(state_t& state) {
    ...
    */
    if (randomer::rand() < 0.5) {
        ...
        state[0].real(randomer::normal(init_y, sigma_y)); 
        state[1].real(randomer::normal(init_x, sigma_x)); 
        state[2].real(randomer::normal(init_py, sigma_py) / mass); 
        state[3].real(randomer::normal(init_px, sigma_px) / mass); 
        state[4].real(sqrt(1.0 - init_s));
        state[5].real(sqrt(init_s));
        state[6].real((randomer::rand() < init_s) ? 1.0 : 0.0);
    }
